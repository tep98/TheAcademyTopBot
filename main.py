...
def start(message):
    ...
    #Отправка сообщения
    bot.send_message(message.chat.id, f"{welcome_word()} {message.from_user.first_name}!\n\nВыберете действие:\n<b>1.</b> Подсчитывать количество проведенных пар по всем дисциплинам у группы\n<b>2.</b> Поиск студентов с средним баллом ниже чем 3", reply_markup=markup, parse_mode = "HTML")
        ...
        if file_name.endswith('.xlsx'):
            try:
                ...
                #Действие 2 (Задание 5)
                    ...
                    bot.send_message(message.chat.id, analyze_average_score(message, chat_id, sheet))
...
#Действие 1 (Задание 7)
def count_complited_classes(message, sheet):
...
#Действие 2 (Задание 5)
def analyze_average_score(message, chat_id, sheet):
